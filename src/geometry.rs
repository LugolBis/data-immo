...
fn clip_polygon(polygon: &Vec<Vec<f64>>, split_value: f64, axis: &str, side: &str) -> Result<Vec<Vec<Vec<f64>>>, String> {
    let mut result: Vec<Vec<f64>> = Vec::new();
    let s = polygon.get(polygon.len()-1).ok_or(())
        .map_err(|_| format!("Failed to get the last element of the polygon : polygon size : {}", polygon.len()))?;

    let mut point_s: [f64;2] = s[..].try_into()
            .map_err(|_| format!("The following point hasn't the expected format : {:?}", s))?;

    for point in polygon {
        let point: [f64;2] = point[..].try_into()
            .map_err(|_| format!("The following point hasn't the expected format : {:?}", point))?;
...
pub fn split_geometry(geometry: &Value) -> Result<(Value, Value), String> {
    let geometry = geometry.as_object()
        .ok_or("Inconsistant Value in input. The input need to be a Value::Object().")?;

    if geometry.get("type").unwrap_or_default() != "Polygon" {
        return Err("Inconsistant geometry type, this function only support Polygon type.".to_string())
    }
    ...
    let exterior = coordinates.get(0)
