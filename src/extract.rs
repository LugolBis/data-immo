...
async fn api_post(
    ...
    let url = format!("https://api.sogefi-sig.com/{}/dvfplus/v1.0/sogefi/{}", api_key, endpoint);
    ...
    match response.status().as_u16() {
        200 => Ok(response),
        402 => Err(format!("Failed of POST '{}' - 402 : Ecxceed request quota", url)),
        status => {
            let reason = response
                .text()
                .await
...
fn get_department(file_path: PathBuf) -> Result<Value, ()> {
    ...
    let value: Value = serde_json::from_str(&buffer)
        .map_err(|e| error!("Failed to convert the '{}' content to a Value : {}", file_path.display(), e))?;
    Ok(value)
}
...
async fn process_features(features: Vec<Map<String, Value>>, api_key: &str, headers: &HeaderMap, dpt: usize) -> Result<(), ()> {
    for index in 0..features.len() {
        let geometry = features.get(index).ok_or(())
        ...
        if let Ok(_) = save_mutation_feature(&feature_id, api_key, headers, data).await {
            info!("Successfully save the mutation from the dpt {} and feature {}", dpt, index);
        }
        else {
            error!("Failed to save the mutation from the dpt {} and feature {}", dpt, index);
        };
    }
    Ok(())
}
...
pub async fn main(folder_path: &str) -> Result<String, String> {
    ...
    if !fs::exists(&target_folder).unwrap_or(false) {
        let _ = fs::create_dir_all(&target_folder)
            .map_err(|e| format!(
                "Failed to create the folder {} : {}", target_folder.display(), e
            ))?;
    }

    let entries = fs::read_dir(&folder_path)
        .map_err(|e| format!(
            "Failed to read the folder {} : {}", folder_path.display(), e
        ))?
        .flatten()
        ...
        if path.is_file() {
            if let Ok(Value::Object(map)) = get_department(path.clone()) {
                let features = map.get("features")
                    .ok_or(format!("Failed to get the 'features' from {}", path.display()))?
                    .as_array()
                    .ok_or(format!("Failed to get the Array from the 'features' Value."))?
                    .iter()
                    .map(|v| {
                ...
                let _ = process_features(features, &api_key, &headers, dpt).await;
            }
            dpt += 1;
        }
        else {
